...
public class AgentHandler implements RequestHandler<Map<String, Object>, Response> {
  ...
  public int runAgent(Map<String, Object> event) throws IOException, InterruptedException {
    ...
    if (event.get("command") != null) {
      ...
      String nodeSecret =
          (event.get("node_secret") != null) ? (String) event.get("node_secret") : "";
      // real parameters for jenkins jnlp slave execution
      // command = "java -cp .:/opt/java/lib/* -Duser.home=" + HOME_DIR + "
      // -Dhudson.remoting.Engine.socketTimeout=1000
      // -Dorg.jenkinsci.remoting.engine.JnlpProtocol3.disabled=true
      // -Dhudson.remoting.SynchronousCommandTransport.failOnSocketTimeoutInReader=true
      // hudson.remoting.jnlp.Main -noreconnect -headless -url " + url + " " + nodeSecret + " " +
      // nodeName;

      command =
          "java -cp /var/task/org.jenkins-ci.main.remoting-4.3.jar:/var/task/lib/*"
              + " -Duser.home="
              + HOME_DIR
              + " -Dhudson.remoting.Engine.socketTimeout=1000"
              + " -Dorg.jenkinsci.remoting.engine.JnlpProtocol3.disabled=true"
              + " -Dhudson.remoting.SynchronousCommandTransport.failOnSocketTimeoutInReader=true"
              + " hudson.remoting.jnlp.Main -noreconnect -headless -url "
              + url
              + " "
              + nodeSecret
              + " "
